...
int test_single(void)
{
    ...
    if (array != NULL)
    {
        ...
        void *data = callocs_malloc(11);
        if (data != NULL)
        {
            printf("success!\n");
            callocs_fprintf_heap_info(stdout);
            callocs_free(data);
            printf("data callocs_freed!\n");
        }
        else
        {
            printf("fail!\n");
...
int test_frag(void)
{
    ...
    int *ptrs[4];
    ptrs[0] = callocs_malloc(16);
    ptrs[1] = callocs_malloc(16);
    ptrs[2] = callocs_malloc(16);
    ptrs[3] = callocs_malloc(16);
    ...
    callocs_free(ptrs[3]);
    printf("callocs_freed last pointer: %p\n", (void *)ptrs[3]);
    callocs_fprintf_heap_info(stdout);
