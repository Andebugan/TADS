...
int callocs_init(size_t size)
{
    ...
    // use original callocs_malloc only once
    mem_pool = callocs_malloc(size);
    if (mem_pool == NULL)
        return -1;
...
{
    size *= num;
    void *data = callocs_callocs_malloc(size);
...
{
    if (ptr == NULL)
        return size == 0 ? NULL : callocs_callocs_malloc(size);
    ...
    else
    {
        ...
        if (node->size < size)
        {
            ...
            // 2. look for prev callocs_free block
            ...
            else // 3. look for completely new callocs_free space
            {
                void *res = callocs_callocs_malloc(size);
                if (res == NULL)
...
int callocs_fprintf_heap_info(FILE *file)
{
    ...
    for (struct __meta_node *node = mem_pool; node != NULL; node = node->next)
    {
        if (node->allocated)
            alloc_blocks_count++;
        fprintf(file, "  %s block ", (node->allocated ? "allocated" : "callocs_free"));
